...
- [ceramic/](ceramic/) — Ceramic (ceramic.ai). One `/search` endpoint, `Authorization: Bearer` auth, base `call()` fits (plain JSON POST). Params: `query` (1–50 words) + optional `maxDescriptionLength` (1000–8000). It's **keyword** search (matches distinctive terms, not natural language / semantic), so it complements the semantic engines. Results nest under `body.result.results` (`{title,url,description}`); the renderer maps that path + `description` snippet. Reports its own processing time at `result.searchMetadata.executionTime` (seconds) → surfaced as the "server" latency pill.
...
- **Server-side timing (the "server" latency pill).** Probed each provider's live response for a self-reported processing time. Reported (surfaced next to round-trip in `app/playground.js` via `extractServerTime`): **Exa** `searchTime` (ms), **You.com** `metadata.latency` (s), **Tavily** `response_time` (s), **SerpApi** `search_metadata.total_time_taken` (s), **Ceramic** `result.searchMetadata.executionTime` (s). Units are normalized to ms. **Not reported** (no pill): Perplexity, Parallel, Brave — neither body nor headers carry a processing-time field. Kept as response-shape logic in the frontend helper (like `extractCost`) rather than the catalog, for consistency.
